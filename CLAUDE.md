...
### 2. Decoy generation via protein-level shuffling

`_shuffle_protein(seq, random_state=42)` in [candidates.py](ms1rescore/candidates.py) shuffles all residues of the protein sequence randomly (using a seeded RNG for reproducibility), then digests the shuffled sequence with the same trypsin rules as the target.

**Why shuffling instead of K/R-preserving reversal:** K/R-preserving reversal frequently produces decoy peptides that are isobaric with targets — the same mass (because K/R are preserved) and often near-identical elemental composition (because non-K/R residues are just reordered without changing counts). This makes isotope envelope features (`theo_isotope_cosine`, `theo_isotope_chi2`, `theo_isotope_kl`, `isotope_envelope_*`) non-discriminative, since isobaric targets and decoys have nearly identical predicted isotope patterns. Random shuffling breaks elemental composition conservation across the peptide level, making these features genuinely informative.

Note: shuffled decoys will still naturally produce tryptic peptides ending in K/R at the background frequency (K/R make up ~11% of human proteins), so the charge distribution is approximately preserved without needing to fix their positions.
...
Two-phase pipeline:
1. **Phase 1 (pyteomics):** `pyteomics.parser.cleave()` for tryptic digestion. Decoys generated by `_shuffle_protein()` (random residue shuffle, seeded at 42).
...
**`LCMS_PRIOR_FEATURES`** — features derived from LC-MS/MS mzML data:
`lcms_ms2_spectral_angle`, `lcms_ms2_n_matches`, `lcms_xic_max_intensity`, `lcms_xic_n_scans`, `lcms_xic_snr`, `lcms_xic_best_charge`, `lcms_rt_residual`, `lcms_ms1_isotope_cosine`, `theo_m1_ratio_diff_lcms`, `theo_m2_ratio_diff_lcms`

**Design rationale:** LC-MS/MS features are explicitly excluded from the ranker/SVM training set. Passing them to the model would cause it to score LC-MS/MS identification quality rather than MALDI match quality — i.e., it would learn to distinguish well-identified LC-MS/MS peptides from everything else, which is not what we want. Instead, LC-MS/MS evidence is applied as a multiplicative Bayesian prior *after* MALDI-intrinsic scoring (see `compute_lcms_prior()` in `pipeline.py`).
